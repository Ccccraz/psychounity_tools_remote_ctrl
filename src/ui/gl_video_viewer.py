...
class GLVideoViewer(QWidget):
    ...
    def load_resource(self, source: str, fps: int | None = None) -> None:
    ...
    def play(self) -> None:
        ...
        self.stream = CamGear(source=self.source, colorspace="COLOR_BGR2RGB").start()
...
if __name__ == "__main__":
    ...
    class MainWindow(QMainWindow):
        def __init__(self) -> None:
            super().__init__()
            self._init_ui()
            self.viewer.load_resource(r"assets\test_anuimal.mp4")
            self.viewer.play()
